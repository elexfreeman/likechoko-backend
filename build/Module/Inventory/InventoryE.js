...
class InventoryE {
    getRulesInsert() {
        ...
        rules.set(rules.rule('checkout').type(lib_2.ModelRulesT.int).moreOrEq(0).less(2).error('checkout - неверный формат'));
        ...
        rules.set(rules.rule('storehouse_id').type(lib_2.ModelRulesT.int).require().more(0).error('storehouse_id - неверный формат'));
        return rules.get();
    }
    getRulesUpdate() {
        let rules = new lib_1.Components.ModelRulesC();
