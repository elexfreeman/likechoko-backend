...
function update(req, data) {
  ...
  rules.set(
    rules.rule('description').type(Components.ModelRulesT.text).maxLen(100).errorEx('description', 'description'),
  );
  rules.set(rules.rule('var_type').type(Components.ModelRulesT.int).moreOrEq(0).errorEx('var_type', 'var_type'));
  // ---------------------------------------
  let validator = new Components.ModelValidatorSys(req.sys.errorSys);
  validator.fValid(rules.get(), data);
  return validator.getResult();
...
//# sourceMappingURL=ProductCategoryVarV.js.map
