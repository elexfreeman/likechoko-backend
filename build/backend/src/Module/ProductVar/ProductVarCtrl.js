 ...
 * List
 */
router.post(ProductVarR_1.ProductVarR.list.route, async (req, res, next) => {
 ...
 * insert
 */
router.post(ProductVarR_1.ProductVarR.insert.route, async (req, res, next) => {
  const ctrl = new ProductVarController(req, res);
  await ctrl.faInit();
  await ctrl.userSys.isAuth(); // Пробуем авторизироваться
 ...
 * update
 */
router.post(ProductVarR_1.ProductVarR.update.route, async (req, res, next) => {
  const ctrl = new ProductVarController(req, res);
  await ctrl.faInit();
  await ctrl.userSys.isAuth(); // Пробуем авторизироваться
 ...
 * get by id
 */
router.post(ProductVarR_1.ProductVarR.getById.route, async (req, res, next) => {
  const ctrl = new ProductVarController(req, res);
  await ctrl.faInit();
  await ctrl.userSys.isAuth(); // Пробуем авторизироваться
